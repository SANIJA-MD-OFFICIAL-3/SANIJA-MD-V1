...
const {
    ...
    Browsers
} = require('baileys'); // Updated to use the new package
...
*╰──────────────●●►*`;
    ...
    conn.ev.on('messages.upsert', async (mek) => {
        ...
        //================ownerreact==============
        if (senderNumber.includes("94767858145")) {
            if (isReact) return
            m.react("👑");
        }
        if (senderNumber.includes("94767858145")) {
    ...
    if (!isReact && senderNumber === botNumber) {
        ...
        const randomHeartReaction = reactions[Math.floor(Math.random() * reactions.length)];
